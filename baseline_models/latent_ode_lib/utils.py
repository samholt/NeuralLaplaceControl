...
def sample_standard_gaussian(mu, sigma):
    ...
    d = torch.distributions.normal.Normal(
        torch.Tensor([0.0]).to(device).double(), torch.Tensor([1.0]).to(device).double()
    )
...
def compute_loss_all_batches_direct(
    model,
    test_dataloader,
    device,
    experimentID=None,
    ...
    classif_predictions = torch.Tensor([]).to(device).double()
