...
def plot_test(ctrl, D, N=None, H=None, L=10, fname=None, verbose=False, savefig=True):
    with torch.no_grad():
        ...
        g, st, at, rt, tobs = D.extract_data(H, ctrl.is_cont)  # pyright: ignore [reportOptionalMemberAccess]
...
def plot_sequences(
    ...
    plt.close()
    [L, N, T, _] = st_hat.shape  # pylint: disable=unused-variable
...
def train_loop(ctrl, D, fname, Nround, **kwargs):
    ...
    for round_ in range(r0, r0 + Nround):
        print(f"Round {round_}/{r0+Nround} starting")
        # dynamics training
        dyn_tr_fnc(ctrl, D, fname, verbose, print_times, **kwargs)  # pyright: ignore
        # policy learning
        train_policy(
            ctrl,
            D,
            H=H,
            V_const=min(round_ / 5.0, 1),
            verbose=verbose,
            Niter=N_pol_iter,
            L=L,
            save_fname=fname,
        ...
        if (test_rewards[..., Tup:] < 0.8).sum() == 0:
            print(f"All {Ntest} tests are solved in {round_} rounds!")
...
def train_policy(
    ...
    for itr in range(Niter):
        ...
        [L, N_, Hdense, n] = st.shape  # pylint: disable=unused-variable
        gammas = (-ts / tau).exp()  # H
        ...
        with torch.no_grad():
            ...
            # L,N,T-1
            last_values = Vtarget(last_states).squeeze(-1)  # pyright: ignore
        ...
        print_log += "first/final value error:{:.3f}/{:.3f}  kl:{:.3f}".format(
            first_val_err, td_error.item(), ctrl.V.kl().item()  # pyright: ignore
        )
...
def dynamics_loss(ctrl, st, ts, at, g, L=1):
    ...
    [N, T, n] = st.shape  # pylint: disable=unused-variable
    [L, N, T, _] = st_hat.shape  # pylint: disable=unused-variable
...
def get_kernel_interpolation(env, T, N=1, ell=0.25, sf=0.5):
    with torch.no_grad():
        ...
        smooth_noise = torch.stack(
            [obtain_smooth_test_acts(env, T, sf=sf, ell=ell) for _ in range(N)]  # pyright: ignore
        )  # N,T,1
...
def collect_data(env, H, N=1, sf=0.5, ell=0.5, D=None):
    ...
    with torch.no_grad():
        ...
        for i in range(N):
            ...
            st, at, rt, ts = env.integrate_system(T, g, s0[i : i + 1])  # pylint: disable=unused-variable
...
def collect_test_sequences(ctrl, D, N=1, reset=True, explore=False, sf=0.1):
    with torch.no_grad():
        ...
        for i in range(N):
            ...
            st_obs, at, rt, ts = env.integrate_system(T, g, s0[i : i + 1])  # pylint: disable=unused-variable
...
def collect_experience(ctrl, D, N=1, H=None, reset=False, explore=False, sf=0.1):
    with torch.no_grad():
        ...
        D.add_experience(Dnew.sa, Dnew.ts)  # pyright: ignore
        return D
...
class TemperedOpt:
    def __init__(self, OPT_CLS, params, lr):
        ...
        my_lambda = lambda ep: min(10.0, 10 ** (ep / 100)) / 10  # noqa: E731
...
def get_ivs(env, D, N, rep_buf=None):
    if rep_buf is None and D is not None:
        rep_buf = D.N
    seq_idx = np.arange(D.N)[-rep_buf:]  # pylint: disable=invalid-unary-operand-type  # pyright: ignore
