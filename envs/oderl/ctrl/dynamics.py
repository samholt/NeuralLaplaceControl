...
class Dynamics(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(
        ...
        L,
        nl=2,
        nn=100,  # pylint: disable=redefined-outer-name
        act="relu",
        dropout=0.0,
        bnn=False,
    ):
    ...
    def kl(self):
        try:
            return self._f.kl()  # pyright: ignore
...
class NODE(Dynamics):  # pylint: disable=abstract-method
    def __init__(
        self,
        env,
        dynamics,
    ...
    def _forward_simulate(self, solver, ts, t0s, s0, f, g, L, tau=None, compute_rew=True):
        T = len(ts) - 1
        # N,n:
        [N, n] = s0.shape  # pylint: disable=unused-variable
        ...
        odef = lambda t, s: self.odef(t, s, f, g, t0s, tau=tau, compute_rew=compute_rew)  # noqa: E731
        strt = odesolve(
            odef,
            s0r0,
            ts,
        ...
        # L,N,T,n & L,N,T:
        st, rt = strt[:T, ..., :n].permute([1, 2, 0, 3]), strt[:T, ..., -1].permute([1, 2, 0])  # pyright: ignore
        return st, rt, self.at


class PETS(Dynamics):  # pylint: disable=abstract-method
    def __init__(
        self,
        env,
        dynamics,
    ...
    def _forward_simulate(self, solver, ts, t0s, s0, f, g, L, tau=None, compute_rew=True):
        ...
        for t_, delta_t_ in zip(ts, delta_t):  # 0 & 0.1
            ...
            s = st[-1] + delta_t_ * ds
            st.append(s)
            self._f.shuffle()  # pyright: ignore
...
class DeepPILCO(Dynamics):  # pylint: disable=abstract-method
    def __init__(
        self,
        env,
        dynamics,
