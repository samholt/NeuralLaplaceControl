...
class CTRL(nn.Module):  # pylint: disable=abstract-method
    def __init__(self, env, dynamics, **kwargs):
        ...
        self.V = BNN(
            self.env.n,
            1,
            n_hid_layers=kwargs["nl_V"],
            act=kwargs["act_V"],
            n_hidden=kwargs["nn_V"],
            bnn=False,
        )
    ...
    def draw_f(self, L=1, noise_vec=None, true_rhs=False):
        ...
        return self._f._f.draw_f(L, noise_vec)  # pyright: ignore  # TODO - check if mean is a parameter
