...
class CTCartpole(BaseEnv):  # pylint: disable=abstract-method
    """
    x, x_dot, theta, theta_dot = self.state
    """

    metadata = {"render.modes": ["human", "rgb_array"], "video.frames_per_second": 50}  # pyright: ignore
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(
            low=-np.array([5.0, 20.0, 1.0, 1.0, 30.0]),  # pyright: ignore
            high=-np.array([5.0, 20.0, 1.0, 1.0, 30.0]),  # pyright: ignore
            dtype=np.float64,
        )
    ...
    def df_du(self, state):
        if state.shape[-1] == 4:
            ...
            costheta = costheta / C
            sintheta = sintheta / C
        den = self.length * (4.0 / 3.0 - self.masspole * costheta * costheta / self.total_mass)  # pyright: ignore
        d2 = -self.force_mag * costheta / self.total_mass / den  # pyright: ignore
        d1 = (self.force_mag - d2 * self.polemass_length * costheta) / self.total_mass  # pyright: ignore
        if state.shape[-1] == 4:
            return torch.stack([x * 0.0, d1, x * 0.0, d2], -1)  # pyright: ignore
        elif state.shape[-1] == 5:
            return torch.stack([x * 0.0, d1, x * 0.0, x * 0.0, d2], -1)  # pyright: ignore
    ...
    def obs2state(self, obs):  # pylint: disable=arguments-renamed
        if obs.shape[-1] == 4:
            return obs
        x, x_dot, costheta, sintheta, theta_dot = (
            obs[..., 0],
            obs[..., 1],
            obs[..., 2],
            obs[..., 3],
        ...
        if fiveD:
            return torch.stack(
                [
                    x_dot,
                    xacc,
                    -sintheta * theta_dot / C,  # pyright: ignore
                    costheta * theta_dot / C,  # pyright: ignore
                    thetaacc,
                ],
                -1,
            )
    ...
    def diff_obs_reward_reduced_state(
        ...
        if self.swing_up:
            ...
            if state_constraint:
                ...
                # position_error = ((err[:,0]>0.0).float() * torch.abs(err[:,0]) * 100.0)
                # + ((err[:,0]<0.0).float() * torch.abs(err[:,0]) * 0.25)
    ...
    def render(self, *args, mode="human", **kwargs):
        ...
        if self.viewer is None:
            ...
            self.axle = rendering.make_circle(polewidth / 2)  # pyright: ignore
