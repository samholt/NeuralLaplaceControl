...
# [CTPendulum, CTCartpole, CTAcrobot]:
ENV_CLS = envs.CTAcrobot  # pyright: ignore
env = ENV_CLS(
    dt=dt,
    obs_trans=True,
    device=device,
...
display = pyvirtualdisplay.Display(visible=0, size=(1400, 900)).start()  # pyright: ignore
...
def g(state, t):  # pylint: disable=redefined-outer-name
...
with imageio.get_writer(filename, fps=fps) as video:
    for iter_ in tqdm(range(100)):
        ...
        video.append_data(env.render(mode="rgb_array", last_act=returns[1][-1]))  # pyright: ignore
env.close()
