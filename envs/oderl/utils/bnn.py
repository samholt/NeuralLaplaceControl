...
class BNN(nn.Module):
    ...
    def reset_parameters(self, gain=1.0):
        for i, (weight, bias) in enumerate(zip(self.weight_mus, self.bias_mus)):  # pylint: disable=unused-variable
            nn.init.xavier_uniform_(weight, gain)
            fan_in, _ = nn.init._calculate_fan_in_and_fan_out(weight)
            bound = 1 / np.sqrt(fan_in)
            if self.bias:
                nn.init.uniform_(bias, -bound, bound)
        for norm in self.norms[:-1]:  # pyright: ignore
    ...
    def __sample_weights(self, L, noise_vec=None):
        if self.bnn:
            ...
            for weight_mu, weight_sig in zip(self.weight_mus, self.weight_logsigs):
                ...
                weights.append(
                    weight_mu
                    + noise_vec[:, i : i + p].view(L, *weight_mu.shape)  # pyright: ignore
                    * self.__transform_sig(weight_sig)
                )
                i += p
            if self.bias:
                biases = []
                for bias_mu, bias_sig in zip(self.bias_mus, self.bias_logsigs):
                    ...
                    biases.append(
                        bias_mu
                        + noise_vec[:, i : i + p].view(L, *bias_mu.shape)  # pyright: ignore
                        * self.__transform_sig(bias_sig)
                    )
                    i += p
            else:
                biases = [
