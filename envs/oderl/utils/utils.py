...
def sq_dist(X1, X2, ell=1.0):
    ...
    sq_dist = -2 * torch.mm(X1, X2.t()) + X1s + X2s  # pylint: disable=redefined-outer-name
    return sq_dist
...
def sq_dist3(X1, X2, ell=1.0):
    ...
    sq_dist = -2 * X1 @ X2.transpose(-1, -2) + X1s + X2s  # pylint: disable=redefined-outer-name
    return sq_dist
...
def batch_sq_dist(x, y, ell=1.0):
    ...
    Input: x is a bxNxd matrix y is an optional bxMxd matrix
...
def log_sum_exp(value, dim=None, keepdim=False):
    ...
    if dim is not None:
        ...
        if isinstance(sum_exp, torch.tensor):  # pyright: ignore
