...
class DropoutBNN(nn.Module):
    ...
    def reset_parameters(self, gain=1.0):
        for i, (weight, bias) in enumerate(zip(self.weights, self.biases)):  # pylint: disable=unused-variable
            nn.init.xavier_uniform_(weight, gain)
            fan_in, _ = nn.init._calculate_fan_in_and_fan_out(weight)
            bound = 1 / np.sqrt(fan_in)
            nn.init.uniform_(bias, -bound, bound)
        for norm in self.layer_norms[:-1]:  # pyright: ignore
    ...
    def draw_noise(self, L=1):
        ...
        for h in self.layers_dim[1:-1]:
            dropout_masks.append(b.sample([L, 1, h]).to(self.device))  # pyright: ignore
